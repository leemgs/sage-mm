...
experiments/
  actual_factorial_targets.csv # prospective targets, never measured results
docs/
...
The measured-result ledger for real ARM32/ARM64 Mono/Tizen (or a real .NET
runtime), including EventPipe/vendor-GC fields, `smaps` `Private_Clean`
accounting, 30-minute runs, and eight-hour endurance, is in
[`paper/docs/MEASURED_DEVICE_RUN_VALUES.md`](paper/docs/MEASURED_DEVICE_RUN_VALUES.md).
...
- `ReclamationCandidateTracker` defines a reproducible recency/frequency/size coldness score and selects K from a byte budget. The demo observes loaded modules, uses file length only as a selection-size proxy, and sends eligible paths to `FlushModule`; this is wiring validation, not evidence of clean bytes reclaimed. A production integration must obtain runtime access events, verify `Private_Clean` from `/proc/self/smaps`, and apply its executable-module allowlist.
