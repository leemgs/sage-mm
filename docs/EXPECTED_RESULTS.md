# Prospective expected values for the additional experiments

> **NOT MEASURED RESULTS.** The numbers below are preregistered engineering hypotheses, normalized to `Stock=100`. They are included because the revision requires explicit expected values before additional experiments. They must be replaced by observed run-level estimates and 95% confidence intervals, and must not be quoted in the abstract, Results, or conclusion as achieved improvements.

## Factorial expectations

The complete machine-readable 2×2×2×2 target matrix is `experiments/expected_factorial_targets.csv`. `G`, `I`, `R`, and `C` denote heap configuration, interop conversion, page reclamation, and coordinated controller. Lower indices are better. We expect the full `1111` treatment to reach PSS 77, allocation rate 72, GC p99 64, fault rate 125, and input p99 82 relative to Stock 100, with controller CPU below 1.5%. The deliberately adverse expectation is that unguarded/static reclamation (`R=1,C=0`) raises fault-rate index to 160; successful coordination should hold it near 125.
...
## Controller-policy expectations with G/I/R enabled
...
The policy hypothesis is modest: Ridge is expected to improve PSS by only 1 point and input p99 by 2 points over EWMA. If its 95% CI overlaps EWMA or overhead exceeds 1.5%, the paper must report that ML offers no demonstrated advantage and prefer the simpler policy.
...
## Additional-platform expected range

On the independently assembled constrained ARM64 Linux platform, the expected full-system range is PSS index 75–85, GC p99 index 60–75, input p99 index 78–90, fault index 115–140, and controller CPU ≤1.5%. Results outside this range are still reported; they narrow generality rather than being discarded.

## Reporting rule

Observed data replace—not silently overwrite—these hypotheses. The final table must show `Expected`, `Observed effect`, `95% CI`, `n`, and `Pass/Fail` columns. All paper claims must be generated from the versioned observed result bundle. A projected target is never labeled “result,” “improvement,” or “deployment evidence.”
