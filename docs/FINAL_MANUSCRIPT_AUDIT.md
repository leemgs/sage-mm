...
8. **The quantitative pipeline is partial.** A run-level CSV contract, deterministic bootstrap summarizer, complete prospective factorial matrix, and target validator exist. No workload driver, observed trace, canonical observed result file, figure generator, or measured table generator is present, so the consistency rule cannot yet be applied to real claims.
