...
| Quantitative component ablation / Table V (R1.4, R2.3, R3.4, R4.3) | **Open—evidence required** | A 16-cell factorial schema and CI method are specified. | Run the combinations under identical randomized/reset conditions and replace actual-impact cells with measured effects, uncertainty, and mechanism costs. No such data exist here. |
...
4. **The training/reporting boundary and result summarizer exist, but tuning and held-out data are absent.** `BeginReporting()` clears the pending training prediction while retaining weights, and `updateModel:false` freezes reporting. A deterministic run-level bootstrap summarizer and prospective target matrix now exist. There is still no tuning trace, split manifest, model serialization, or observed result bundle. “Tuned threshold” and “held-out” remain experimental requirements, not established results.
...
8. **The quantitative pipeline is partial.** A run-level CSV contract, deterministic bootstrap summarizer, complete prospective factorial matrix, target validator, and fixed-seed simulation generator now exercise RQ prose, figures, 16 CI rows, additional-platform, and adverse/endurance tables. The outputs are visibly marked synthetic. No workload driver, observed trace, canonical observed result file, or measured figure/table generator is present, so the consistency rule cannot yet be applied to real claims.
