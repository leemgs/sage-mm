...
operator reset). This guard, prompted by the adverse result, is evaluated directly
...
The public kit contains a reference controller, policy events, a native mapping
helper, and an advisory analyzer; its console demo is a wiring exercise whose
proxy signals (a forced Gen-0 pause, a pinned-object fragmentation estimate,
process faults/RSS, and module file lengths as size estimates) are \emph{not} the
device evaluation instrument and do not measure reclaimed \texttt{Private\_Clean}
bytes or drive a vendor compaction hook. A device experiment therefore supplies
its own runtime GC traces, live fragmentation accounting, mapping-level
\texttt{smaps} samples, workload event timestamps, and an implemented compaction
hook, with the firmware image, runtime patch, collector revision, controller
configuration, and reset procedure recorded in the manifest and per-run bundle; a
public-kit execution alone does not establish a production deployment.
...
\label{sec:rq1}
The normalized coordination ladder under the favorable workload
(Table~\ref{tab:policy-index}) moves different metrics at each step. The
increment from Stock
...
roughly twice the controller cost. Direct Ridge--EWMA difference intervals
(unpaired bootstrap) include zero for ARM32 GC p99 and ARM64 input p99 but
exclude it for ARM32 input p99 and ARM64 GC p99; consistent with our fail-closed
stance, the threshold policy remains a defensible default when CPU headroom is
tight.
...
collection~\cite{ungar1984scavenging,appel1989generational,boehm1988uncooperative,
blackburn2008immix,detlefs2004g1,bacon2003metronome}, the collected-versus-explicit
trade-off~\cite{hertz2005quantifying}, textbook surveys~\cite{jones1996gc,
jones2011handbook}, and the allocation layer beneath it~\cite{wilson1995survey,
berger2002reconsidering}. Because heap footprint depends strongly on the memory
given to the collector, controlling heap growth and selecting or sizing the
collector are established levers~\cite{brecht2006heapgrowth,soman2004dynamic,
stefanovic1999age}. The footprint-versus-refault tension SAGE-MM manages is the
...
services~\cite{dean2013tail,maas2015trashday,lion2016coldjvm}; SAGE-MM introduces
...
models~\cite{denning1968working,carr1981wsclock}, recency/frequency
replacement~\cite{jiang2002lirs,megiddo2003arc,zhou2001multiqueue}, flash/device-aware
