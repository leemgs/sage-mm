...
\begin{description}
  \item \textbf{RQ1 (Ablation).} Along the coordination ladder---stock, heap
  build ($G$), interop ($I$), reclamation ($R$), controller ($C$)---what
  sequential increment does each step add for peak PSS, GC tail pause, refault
  rate, and input tail latency on the two profiles?
  \item \textbf{RQ2 (Policy).} Under identical bounds, telemetry, cooldown, and
  ...
  and EWMA?
  \item \textbf{RQ3 (Robustness).} Across favorable, neutral, and adverse
