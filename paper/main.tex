...
\title{Coordinated Memory Management for Memory-Constrained
...
\begin{abstract}
Digital televisions (DTVs), set-top boxes, and similar consumer-electronics
appliances increasingly run managed (.NET/CoreCLR) application stacks on
cost-driven, swapless memory budgets. Within one process budget a single
application co-locates a garbage-collected heap, native interoperability
buffers, and file-backed executable code pages; tuned in isolation these layers
trade off against one another and surface to the viewer as channel-change
stutter, slow launch, or an out-of-memory shutdown---a direct
quality-of-experience problem for the device. We present SAGE-MM, a cross-layer
memory-management design that coordinates all three layers under a single
budget: it separates a launch-time managed-heap configuration and a reviewed
source-time interop conversion from one narrow online controller adjusting a
single reclamation interval and compaction gate, with
normalized telemetry, hysteresis, a causal update order, fail-closed handling of
invalid telemetry, a non-learning threshold fallback, and an adverse-regime
supervisor. We reproduced the actual Tizen DTV platform (a Tizen~10.0 image and
its managed runtime) on a Raspberry~Pi~4 Model~B---an official Tizen reference
board---and evaluated it across 32- and 64-bit builds under a $512$\,MiB swapless
budget, with $30$ runs per condition and $95\%$ percentile bootstrap confidence
intervals; every reported number is produced by a fail-closed pipeline from a
per-run bundle. On a favorable workload the coordinated design
lowers peak memory footprint by about $24\%$ and garbage-collection tail pause by
$37$--$38\%$ at roughly $1\%$ controller CPU, stays within noise
where no gain exists, and, in an adverse refault-dominated regime, is contained
by the supervisor within seconds---halving controller CPU and eliminating
