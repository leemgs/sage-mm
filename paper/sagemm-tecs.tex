...
%% zero-cost-publication compliance notes.
%%
%% Cost note: This manuscript uses the DEFAULT (traditional, subscription)
%% ACM publishing model. It does NOT opt into ACM Open / Gold Open Access,
%% so there is no author-facing Article Processing Charge (APC). Do not add
%% `\setcopyright{cc}' or any CC license option, which would signal an
%% OA (paid) route.
%% =====================================================================
\documentclass[acmsmall,screen,nonacm=false]{acmart}
...
%% Traditional (no-fee) publishing model. Left generic until acceptance.
...
%% Convenience macro for values that are DELIBERATELY UNSET: they must be
%% filled from the versioned observed-result bundle before submission, and
%% must never be fabricated. Renders as a visible placeholder in the draft.
\newcommand{\TBM}{\textsf{[to be measured]}}
...
device and a 3\,GiB ARM64 device. Mono and CoreCLR are distinct runtime
...
is falsifiable: uncoordinated static reclamation is expected to inflate the
...
modification. The intervention reduces allocation rate and GC pressure
...
value is $-\text{errno}$. Callers retain the mapping after failure. Because
...
confidence interval. We analyze main and interaction effects; expected
...
Every effect below is generated from the single versioned observed-result
bundle and names its baseline, workload aggregation, statistic, denominator,
and confidence interval. Quantities not yet measured are shown as \TBM{} and
must not be quoted as achieved improvements; RSS, PSS, managed heap, and
\texttt{Private\_Clean} are never interchanged.

\subsection{Preregistered expectations (hypotheses, not results)}
\label{sec:expected}
To keep the design falsifiable, Table~\ref{tab:expected} states
preregistered engineering \emph{expectations} normalized to
...
\begin{table}[t]
  \caption{\textbf{Preregistered expectations, not measured results.}
  Normalized to $\textsf{Stock}=100$; lower is better. The key falsifiable
  expectation is that unguarded static reclamation (\textsf{Static-GIR})
...
\subsection{RQ1: Architecture-specific heap configuration}
The ARM32/ARM64 heap sweep reports peak/mean PSS, collection and compaction
counts, and p50/p95/p99/max pause per architecture. The claimed
ARM32/ARM64 compaction-cost ratio is annotated directly on its figure when
regenerated data support it. Observed effect: \TBM{}.

\subsection{RQ2: Representation and guarded reclamation}
The factorial rows isolate interop ($I$) and reclamation ($R$) effects on
allocation rate, private-clean bytes reclaimed, PSS, faults, and reload and
interaction latency, with hot-reuse stress. The safety criterion is that,
relative to \textsf{Static-GI}, guarded reclamation increases the fault
rate by at most 35\% and storage bytes read by at most 10\%, and does not
regress input/frame p99 by more than 5\%. Observed effect: \TBM{}.

\subsection{RQ3: Online coordination and policy comparison}
Held-out traces compare \textsf{Static-GIR}, \textsf{Threshold-GIR},
\textsf{EWMA-GIR}, and \textsf{Ridge-GIR} on OOM count, time-to-OOM,
p95/p99 latency, controller overhead, and causal prediction loss. The
policy hypothesis is modest: ridge is expected to improve PSS by roughly one
point and input p99 by two points over EWMA. If its 95\% CI overlaps EWMA
or its overhead exceeds 1.5\%, we report that the lightweight ML policy
offers no demonstrated advantage and prefer the simpler policy. Observed
effect: \TBM{}.
...
SAGE-MM shows that heap configuration, interop allocation, and clean-page
...
memory, latency, or reliability claims. Numerical placeholders marked
\TBM{} in this draft remain unset until a versioned observed-result bundle
